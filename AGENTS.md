...
| 编辑器 | `input_ui/editor/`（`editor` feature 门控） | `EditorState`（包 `Entity<InputState>` 编排外壳 + 大纲缓存；透传 `set_value`/`reveal_*`/`set_read_only`/`set_line_comment_prefix`/`set_highlighter`/`goto_symbol`）、`Editor`（复用 `Input` + 状态行）；行操作/多光标/自动闭合/括号匹配/当前行（`impl InputState`，见 `docs/1.2.0/1.2.0-dev-plan.md` §M） |
