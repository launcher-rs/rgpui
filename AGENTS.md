...
| 编辑器 | `input_ui/editor/`（`editor` feature 门控，蕴含 `lsp`） | `EditorState`（包 `Entity<InputState>` 编排外壳 + 大纲缓存；透传 `set_value`/`reveal_*`/`set_read_only`/`set_line_comment_prefix`/`set_highlighter`/`goto_symbol`）、`Editor`（消费 `EditorState` 实体 + 粘性大纲顶栏 + 状态行）；行操作/多光标/自动闭合/括号匹配/当前行（`impl InputState`）；LSP 接线（`lsp_attach.rs`：provider 注入 + 防抖 + 诊断下划线）+ 片段（`snippets.rs`：子集解析 + 会话跳转）+ 行内提示（`inlay_hints.rs`：provider + 防抖 + paint overlay）+ 粘性顶栏（`sticky_scroll.rs`）+ 多语言注册表（`highlight::{register_highlighter,highlighter_for}` + `set_language`）+ 自定义 gutter（`gutter.rs`：具名 provider 注册/开关 + overlay，见 `docs/1.2.0/1.2.0-dev-plan.md` §M） |
