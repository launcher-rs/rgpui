...
> | 构建 | 优化前 | 优化后 | 说明 |
> |------|--------|--------|------|
> | `release`（日常） | 11.41 MB | 10.15 MB | `-11%`，默认 `cargo build --release` 直接受益 |
> | `size`（打安装包） | — | 6.69 MB | 相对基线 `-41%`，见 §4 |
...
- 日常开发/CI 继续用 `release`：构建快，体积 10.15MB。
- 发版打安装包用 `cargo build --profile size -p <pkg>`：构建慢（约 7–10 分钟），体积 6.69MB。
...
| image 剩余 14 种解码 | 图片解码 | 可二次裁剪，见 §5 |
| icu + url + idna | `Url` 类型等 | 动即 API 破坏，不做 |
| regex unicode 表 | `\p{Emoji}` 等需要 unicode 支持 | 裁了就错，不做 |
| async 全家桶（smol/async-std/tar/zip） | 命令执行、压缩包、调度 | 架构级依赖，不做 |
| backtrace + object/gimli | `TestScheduler` 帧级回溯 | 做不了，见 §5 |
...
经验值：6.69MB 的 exe 经安装包工具（NSIS/WiX）的 LZMA 压缩后约 3.5–4MB。**觉得安装包大时，先看压缩选项，再看二进制**，前者往往更有效。

## 5. 将来的优化点

### 5.1 可做：image 二次裁剪（预计 100–300KB）

`dds` / `exr` / `hdr` / `tga` / `ff` / `qoi` 在桌面 UI 里极为罕见。做法参考本轮 avif：workspace 显式列表里拿掉，`rgpui` 加 `image-exr = ["image/exr"]` 这类 opt-in 特性。注意 `platform.rs` 的 `ImageFormat` 枚举和解码 match 有 `Tiff`/`Pnm` 等分支——枚举变体不受 feature 影响，编译 safe，缺的是运行时解码能力，acceptance 标准是“缺格式时返回干净错误而非 panic”（当前行为已如此）。
...
阈值按 6.69MB 现状上浮 15–20%（8MB），只防回归、不卡正常演进。注意 `size` 构建约 7–10 分钟，建议放独立 job，别塞进主矩阵里拖慢反馈。
