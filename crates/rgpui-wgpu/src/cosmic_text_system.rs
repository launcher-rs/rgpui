...
impl CosmicTextSystemState {
    ...
    fn load_family(
        ...
        let name = rgpui::font_name_with_fallbacks(name, &self.system_font_fallback);
...
fn find_best_match(
    ...
    let target_italic = matches!(
        font.style,
        rgpui::FontStyle::Italic | rgpui::FontStyle::Oblique
    );
...
#[cfg(feature = "font-kit")]
fn font_into_properties(font: &rgpui::Font) -> font_kit::properties::Properties {
    font_kit::properties::Properties {
        style: match font.style {
            rgpui::FontStyle::Normal => font_kit::properties::Style::Normal,
            rgpui::FontStyle::Italic => font_kit::properties::Style::Italic,
            rgpui::FontStyle::Oblique => font_kit::properties::Style::Oblique,
        },
...
mod tests {
    ...
    fn layout_text(text_system: &CosmicTextSystem, text: &str) -> Result<LineLayout> {
        let font_id = text_system.font_id(&rgpui::font("IBM Plex Sans"))?;
        let runs = [FontRun {
            len: text.len(),
            font_id,
        }];
        Ok(text_system.layout_line(text, rgpui::px(14.0), &runs))
    }
    ...
    fn shape_text_with_mixed_direction_paragraphs() -> Result<()> {
        ...
        let text_system = Arc::new(rgpui::TextSystem::new(platform_text_system));
        let window_text_system = rgpui::WindowTextSystem::new(text_system);

        let text: SharedString = "first line\n\u{05d0}\u{001c}A".into();
        let runs = [rgpui::TextRun {
            len: text.len(),
            font: rgpui::font("IBM Plex Sans"),
            ..Default::default()
        }];

        let lines = window_text_system.shape_text(text, rgpui::px(14.0), &runs, None, None)?;
    ...
    fn layout_line_with_font_run_straddling_a_separator() -> Result<()> {
        ...
        let layout = text_system.layout_line(text, rgpui::px(14.0), &runs);
