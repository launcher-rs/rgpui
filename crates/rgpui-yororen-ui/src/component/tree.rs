...
impl Tree {
    ...
    where
        F: 'static + Fn(&ElementId, &ClickEvent, &mut rgpui::Window, &mut rgpui::App),
    {
    ...
    where
        F: 'static + Fn(&ElementId, &rgpui::MouseDownEvent, &mut rgpui::Window, &mut rgpui::App),
    {
...
impl RenderOnce for Tree {
    fn render(self, window: &mut Window, cx: &mut rgpui::App) -> impl IntoElement {
        // If virtualized, render using rgpui::list
        if self.virtualized {
...
impl Tree {
    /// Render the tree using virtualized list (rgpui::list).
    fn render_virtualized(self, window: &mut Window, cx: &mut rgpui::App) -> impl IntoElement {
    ...
    fn render_normal(self, window: &mut Window, cx: &mut rgpui::App) -> impl IntoElement {
        let show_checkbox = self.show_checkbox;
        let indent = self.indent;
