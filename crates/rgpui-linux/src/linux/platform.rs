    ...
    fn screen_capture_sources(
        ...
        sources_tx
            .send(Err(anyhow::anyhow!(
                "rgpui_linux was compiled without the screen-capture feature"
            )))
            .ok();
        sources_rx
    }
        ...
        self.foreground_executor()
            .spawn(async move {
                ...
                let request = match ashpd::desktop::file_chooser::OpenFileRequest::default()
                    ...
                    .modal(true)
                    .title(title)
                    .accept_label(options.prompt.as_ref().map(rgpui::SharedString::as_str))
...
pub(super) fn compositor_gpu_hint_from_dev_t(dev: u64) -> Option<rgpui_wgpu::CompositorGpuHint> {
    ...
    Some(rgpui_wgpu::CompositorGpuHint {
        vendor_id,
        device_id,
    })
}
