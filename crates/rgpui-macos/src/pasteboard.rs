...
impl Pasteboard {
    ...
    unsafe fn new(inner: id) -> Self {
        Self {
            inner,
            text_hash_type: unsafe { ns_string("rgpui-text-hash") },
            metadata_type: unsafe { ns_string("rgpui-metadata") },
        }
    }
...
mod tests {
    ...
    use rgpui::{ClipboardEntry, ClipboardItem, ClipboardString, ImageFormat};
