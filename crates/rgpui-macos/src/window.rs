...
struct MacWindowState {
    ...
    event_callback: Option<Box<dyn FnMut(PlatformInput) -> rgpui::DispatchEventResult>>,
...
impl MacWindowState {
    ...
    fn bounds(&self) -> Bounds<Pixels> {
        ...
        if screen == nil {
            return Bounds::new(point(px(0.), px(0.)), rgpui::DEFAULT_WINDOW_SIZE);
        }
...
impl PlatformWindow for MacWindow {
    ...
    fn draw(&self, scene: &rgpui::Scene) {
        let mut this = self.0.lock();
    ...
    fn gpu_specs(&self) -> Option<rgpui::GpuSpecs> {
        None
    }
