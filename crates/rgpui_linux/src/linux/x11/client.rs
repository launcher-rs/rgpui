...
use rgpui_wgpu::{CompositorGpuHint, GpuContext};

/// DeviceId 参数中选择所有设备的值
pub(crate) const XINPUT_ALL_DEVICES: xinput::DeviceId = 0;

/// DeviceId 参数中选择所有设备组的值
///
/// 发生在组内的事件由组本身发出
///
/// 在 XInput 2 的接口中，这些被称为"主设备"，但该
/// 术语既过时又不清晰
...
struct ScrollAxisState {
    /// 用于查找此轴滚动值的 Valuator 编号
    valuator_number: Option<u16>,
    /// 从滚动单位到行的转换因子
    multiplier: f32,
    /// 用于计算滚动增量的最后滚动值
    ///
    /// 每当可能无效时（设备变更或窗口焦点变更时），此值会被设为 `None`
    /// 逻辑倾向于将此值设为无效，因为后果只是跳过一个滚动事件的增量
    /// 而不将其设为无效的后果是产生较大的无效增量，这对用户来说更明显
...
pub struct X11ClientState {
    ...
    // TODO: 是否可以移除 `modifiers` 的其他更新，使这变得不必要？
    // capslock 逻辑与 modifiers 类似地完成
...
impl X11Client {
    pub(crate) fn new() -> anyhow::Result<Self> {
        ...
        // 安全性：如果 xcb::Connection 始终返回有效的 fd，则这是安全的
        ...
        loop {
                    ...
                    Ok(Some(event)) => {
                        match event {
                            ...
                            Event::KeyPress(key_press) => {
                                ...
                                {
                                    // 如果这个 KeyRelease 离 KeyPress 太近，我们忽略它
                                    // 这表明它是 X11 自动生成的按键重复事件
            ...
            for event in events.into_iter() {
                ...
                match xim_filtered {
                    ...
                    Err(err) => {
                        // 当 xim 服务器在其中一个事件上崩溃时可能会发生这种情况
                        // 崩溃时我们确实会丢失 1-2 个键，因为没有可靠的方法来获取该信息
                        // 幸运的是，当 xim 服务器崩溃时，x11 会在后续按键时向我们发送窗口未找到错误
                        // 因此我们回退到 handle_event
    ...
    fn handle_event(&self, event: Event) -> Option<()> {
        match event {
            ...
            Event::SelectionNotify(event) => {
                ...
                if let Ok(file_list) = str::from_utf8(&reply.value) {
                    ...
                    let input = PlatformInput::FileDrop(FileDropEvent::Entered {
                        ...
                        paths: rgpui::ExternalPaths(paths),
                    });
                    drop(state);
            ...
            Event::FocusOut(event) => {
                ...
                let mut state = self.0.borrow_mut();
                // 将最后滚动值设为 `None`，以便在窗口外滚动时不会创建大的增量（valuator 是全局的）
            ...
            Event::KeyPress(event) => {
                ...
                drop(state);
                window.handle_input(PlatformInput::KeyDown(rgpui::KeyDownEvent {
                    keystroke,
                    is_held: false,
            ...
            Event::KeyRelease(event) => {
                ...
                drop(state);
                window.handle_input(PlatformInput::KeyUp(rgpui::KeyUpEvent { keystroke }));
            }
            Event::XinputButtonPress(event) => {
                ...
                match button_or_scroll_from_event_detail(event.detail) {
                    Some(ButtonOrScroll::Button(button)) => {
                        ...
                        drop(state);
                        window.handle_input(PlatformInput::MouseDown(rgpui::MouseDownEvent {
                            button,
                            position,
                            modifiers,
                    ...
                    Some(ButtonOrScroll::Scroll(direction)) => {
                        drop(state);
                        // 模拟的滚动按钮按下事件与平滑滚动 XinputMotion 事件同时发送
                        // 由于处理这些事件会完成滚动，因此在这里跳过它们
                        if !event
                            .flags
            ...
            Event::XinputButtonRelease(event) => {
                ...
                match button_or_scroll_from_event_detail(event.detail) {
                    Some(ButtonOrScroll::Button(button)) => {
                        ...
                        drop(state);
                        window.handle_input(PlatformInput::MouseUp(rgpui::MouseUpEvent {
                            button,
                            position,
                            modifiers,
                            click_count,
            ...
            Event::XinputMotion(event) => {
                ...
                if event.valuator_mask[0] & 3 != 0 {
                    window.handle_input(PlatformInput::MouseMove(rgpui::MouseMoveEvent {
                        position,
                        pressed_button,
                        modifiers,
                    }));
            ...
            Event::XinputLeave(event) if event.mode == xinput::NotifyMode::NORMAL => {
                ...
                let window = self.get_window(event.event)?;
                window.handle_input(PlatformInput::MouseExited(rgpui::MouseExitEvent {
                    pressed_button,
                    position,
                    modifiers,
                }));
            ...
            Event::XinputGesturePinchBegin(event) => {
                ...
                drop(state);
                window.handle_input(PlatformInput::Pinch(rgpui::PinchEvent {
                    position,
                    delta: 0.0,
                    modifiers,
                    phase: rgpui::TouchPhase::Started,
                }));
            }
...
impl LinuxClient for X11Client {
    ...
    fn screen_capture_sources(
        &self,
    ) -> futures::channel::oneshot::Receiver<anyhow::Result<Vec<Rc<dyn rgpui::ScreenCaptureSource>>>>
    {
        rgpui::scap_screen_capture::scap_screen_sources(&self.0.borrow().common.foreground_executor)
    }
    ...
    fn write_to_primary(&self, item: rgpui::ClipboardItem) {
        let state = self.0.borrow_mut();
        state
            .clipboard
            .set_text(
    ...
    fn write_to_clipboard(&self, item: rgpui::ClipboardItem) {
        let mut state = self.0.borrow_mut();
        state
            .clipboard
            .set_text(
    ...
    fn read_from_primary(&self) -> Option<rgpui::ClipboardItem> {
        let state = self.0.borrow_mut();
        state
            .clipboard
    ...
    fn read_from_clipboard(&self) -> Option<rgpui::ClipboardItem> {
        let state = self.0.borrow_mut();
