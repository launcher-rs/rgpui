...
impl<T> SankeyChart<T> {
    ...
    /// labels 鈥?the layout's `node.value` is in scaled units under a
...
mod tests {
    ...
    fn test_sankey_label_builder() {
        ...
        let label = SankeyLabel::new("b").color(rgpui::red()).font_size(14.);
        assert_eq!(label.color, Some(rgpui::red()));
    ...
    fn test_sankey_chart_raw_throughput() {
        ...
        // (used for labels) must stay in raw units 鈥?the two must differ.
        let sqrt = chart
