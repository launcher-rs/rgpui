...
//! extend beyond the window 鈥?useful for small windows where a GPUI-drawn popup
//! menu would otherwise be cut off.
//!
//! Items carry a GPUI [`Action`], dispatched via [`Window::dispatch_action`]
//! when selected 鈥?the same mechanism the application menu bar and key bindings
...
/// skipped.
impl From<rgpui::Menu> for NativeMenu {
    fn from(menu: rgpui::Menu) -> Self {
        let mut native = Self::new();
        for item in menu.items {
            match item {
                rgpui::MenuItem::Separator => native.items.push(NativeMenuItem::Separator),
                rgpui::MenuItem::Action {
                    name,
                    action,
                    checked,
                    disabled,
                ...
                rgpui::MenuItem::Submenu(submenu) => native.items.push(NativeMenuItem::Submenu {
                    label: submenu.name.clone(),
                    disabled: submenu.disabled,
                    items: Self::from(submenu).items,
                }),
                rgpui::MenuItem::SystemMenu(_) => {}
            }
        }
        native
    }
...
mod tests {
    ...
    fn test_native_menu_icon_asset_resolves_to_bytes() {
        ...
        let image = resolve_icon_image(icon.path_ref(), &rgpui_component_assets::Assets)
