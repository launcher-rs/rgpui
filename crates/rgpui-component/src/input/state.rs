...
pub(crate) struct WhitespaceIndicators {
    /// Shaped line for space character indicator (鈥?
    pub(crate) space: ShapedLine,
    /// Shaped line for tab character indicator (鈫?
...
impl InputState {
    ...
    /// user should still be able to undo the operation 鈥?e.g. formatting.
    pub fn replace_all(
        &mut self,
...
-- Comment 3";
    ...
    fn test_scroll_to_eob_does_not_overshoot_safe_range(cx: &mut TestAppContext) {
        ...
        // Sanity: paint populated `scroll_size` and `input_bounds` 鈥?without
        ...
        // Move cursor to end with downward direction 鈥?same code path as a
            ...
            input.update(cx, |state, cx| {
                ...
                assert!(
                    ...
                    "deferred_scroll_offset.y = {:?} below safe_y_min = {:?} \
                     鈥?paint would jitter (Bug C regression)",
                    deferred.y,
                    safe_y_min,
                );
            });
    ...
    fn test_number_input_normalization(cx: &mut TestAppContext) {
        ...
        cx.update(|window, cx| {
            input.update(cx, |state, cx| {
                state.replace_text_in_range(None, "12銆?", window, cx);
            });
        });
        cx.run_until_parked();
        cx.update(|_, cx| {
        ...
        cx.update(|window, cx| {
            input.update(cx, |state, cx| {
                ...
                state.replace_text_in_range(Some(range), "銆?, window, cx);
            });
        });
        cx.run_until_parked();
        cx.update(|_, cx| {
    ...
    /// caret at the end, and 鈥?like `set_value` 鈥?snaps the view back to the
