...
mod tests {
    ...
    fn drag(
        ...
        from: rgpui::Point<rgpui::Pixels>,
        to: rgpui::Point<rgpui::Pixels>,
    ) {
        drag_through(cx, &[from, to]);
    }

    fn drag_through(cx: &mut VisualTestContext, points: &[rgpui::Point<rgpui::Pixels>]) {
        assert!(points.len() >= 2);
        let from = points[0];
