...
/// wins over ancestor scrollers (e.g. `rgpui::list`) that register their
...
/// consuming it 鈥?a bubbled horizontal delta would get mapped onto a
/// vertical-only ancestor by rgpui's own wheel listener (see #2468).
        ...
        window.with_content_mask(Some(ContentMask { bounds }), |window| {
            ...
            window.on_mouse_event({
                ...
                move |event: &ScrollWheelEvent, phase, window, cx| {
                    ...
                    // `rgpui::list` register their wheel listeners after their
                    ...
                    // delta passes this guard 鈥?even at the scroll edge the
...
mod tests {
    ...
    /// `rgpui::list` item. The list registers its wheel listener after its
    ...
    fn horizontal_scroll_area_covers_viewport_after_scrolled(cx: &mut TestAppContext) {
        ...
        // cover it 鈥?it must not slide away with the scrolled content.
        cx.simulate_event(ScrollWheelEvent {
    ...
    /// The vertical mask nested in a `rgpui::list` ancestor: the list
