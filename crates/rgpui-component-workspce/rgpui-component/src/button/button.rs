        ...
        self.base
            ...
            .when_some(self.toggled, |this, toggled| {
                this.aria_toggled(if toggled {
                    rgpui::accesskit::Toggled::True
                } else {
                    rgpui::accesskit::Toggled::False
                })
            })
...
mod tests {
    ...
    #[rgpui::test]
    fn test_button_toggle_state_is_opt_in(cx: &mut rgpui::TestAppContext) {
        use crate::ElementExt as _;
        use rgpui::{Element as _, IntoElement as _, Render, accesskit::Toggled};
            ...
            fn render(&mut self, _: &mut Window, _: &mut rgpui::Context<Self>) -> impl IntoElement {
                ...
                div().on_prepaint(move |_, window, cx| {
                    let mut toggled_of = |button: Button| {
                        let mut node = rgpui::accesskit::Node::new(Role::Button);
                        button
                            .render(window, cx)
                            .into_element()
    ...
    #[rgpui::test]
    fn test_outline_selected_uses_outline_active_style(cx: &mut rgpui::TestAppContext) {
        cx.update(crate::init);
    ...
    #[rgpui::test]
    fn test_primary_button_uses_gradient_background_tokens(cx: &mut rgpui::TestAppContext) {
        cx.update(crate::init);
        let window = cx.add_empty_window();
        window.update(|_, cx| {
    ...
    #[rgpui::test]
    fn test_outline_primary_keeps_original_depth(cx: &mut rgpui::TestAppContext) {
        cx.update(crate::init);
        let window = cx.add_empty_window();
        window.update(|_, cx| {
            let config = serde_json::from_value::<crate::ThemeConfig>(serde_json::json!({
    ...
    #[rgpui::test]
    fn test_outline_buttons_use_semantic_gradient_tokens(cx: &mut rgpui::TestAppContext) {
        cx.update(crate::init);
        let window = cx.add_empty_window();
        window.update(|_, cx| {
            let config = serde_json::from_value::<crate::ThemeConfig>(serde_json::json!({
    ...
    #[rgpui::test]
    fn test_disabled_outline_buttons_keep_semantic_backgrounds(cx: &mut rgpui::TestAppContext) {
        cx.update(crate::init);
        let window = cx.add_empty_window();
        window.update(|_, cx| {
            let config = serde_json::from_value::<crate::ThemeConfig>(serde_json::json!({
