    ...
    for child in children {
        ...
        for node in child_paragraph.children {
            ...
            if let Some(mut image) = node.image {
                ...
                // rgpui InteractiveText does not support inline images, so
...
mod tests {
    ...
    fn test_inline_html_image_stays_in_markdown_paragraph() {
        ...
        assert_eq!(image.width, Some(rgpui::px(32.).into()));
        assert_eq!(image.height, Some(rgpui::px(32.).into()));
    }
    ...
    impl MarkdownPlugin for TickerPlugin {
        ...
        fn render(
            ...
            rgpui::div().child(node.as_text().to_string())
        }
    }
