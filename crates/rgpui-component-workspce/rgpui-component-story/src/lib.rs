    ...
    F: FnOnce(&mut Window, &mut App) -> E + Send + 'static,
{
    ...
    let options = WindowOptions {
        ...
        window_min_size: Some(rgpui::Size {
            width: px(480.),
            height: px(320.),
        }),
        kind: WindowKind::Normal,
        #[cfg(target_os = "linux")]
        window_background: rgpui::WindowBackgroundAppearance::Transparent,
        #[cfg(target_os = "linux")]
        window_decorations: Some(rgpui::WindowDecorations::Client),
        ..Default::default()
    };
...
pub fn init(cx: &mut App) {
        ...
        let _ = tracing_subscriber::registry()
            ...
            .with(
                tracing_subscriber::EnvFilter::from_default_env()
                    .add_directive("rgpui_component=trace".parse().unwrap()),
            )
            .try_init();
    }
    ...
    #[cfg(not(target_family = "wasm"))]
    {
        // reqwest_client 不再可用，暂时跳过 HTTP 客户端初始化
        // let http_client =
        //     reqwest_client::ReqwestClient::user_agent("rgpui-component/story").unwrap();
    ...
    #[cfg(target_family = "wasm")]
    {
        ...
        let http_client = unsafe {
            rgpui_web::FetchHttpClient::with_user_agent("rgpui-component/story")
                        ...
                        window.defer(cx, |window, cx| {
                            window.open_alert_dialog(cx, |alert, _, _| {
                                alert.title("About").description(markdown(
                                    "rgpui Component Storybook\n\n\
                                    Version 0.1.0\n\n\
                                    https://github.com/launcher-rs/rgpui",
                                ))
                            });
                        });
                    })
...
impl StoryContainer {
    ...
    pub fn height(mut self, height: rgpui::Pixels) -> Self {
...
impl Focusable for StoryContainer {
    fn focus_handle(&self, _: &App) -> rgpui::FocusHandle {
