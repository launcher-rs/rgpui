...
fn main() {
    // Publish the absolute path of `rgpui-component-workspce/rgpui-component-assets/icons` so dependents that need
    // the icons at *build time* (notably `rgpui-component`, whose `IconName`
    ...
    // every dependent's build script 鈥?see the `links` field in our
    // `Cargo.toml` for the full mechanism.
    //
    // Within this crate, `rgpui-component-workspce/rgpui-component-assets/icons` is also consumed at *runtime* by
    // `RustEmbed` (see `src/native_assets.rs`), which finds the files
    // relative to our own `CARGO_MANIFEST_DIR`. So this build script does
    // not need to copy or move anything 鈥?it only advertises the location.
    let manifest_dir = env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR not set by cargo");
    let icons_dir = Path::new(&manifest_dir).join("rgpui-component-workspce/rgpui-component-assets/icons");
    ...
    println!("cargo:rerun-if-changed=rgpui-component-workspce/rgpui-component-assets/icons");
