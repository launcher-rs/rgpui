...
use rgpui::{prelude::*, *};
use rgpui_component::{theme::Theme, Root};
use rgpui_component_assets::Assets;
use rgpui_component_story::{Gallery, StoryRoot};
...
pub fn run() -> Result<(), JsValue> {
    ...
    app.with_assets(Assets::new(
        "http://localhost:3000/rgpui-component/gallery/",
    ))
    .run(|cx: &mut App| {
        ...
        cx.open_window(WindowOptions::default(), |window, cx| {
            ...
            let story_root = cx.new(|cx| StoryRoot::new("rgpui Component", view, window, cx));
