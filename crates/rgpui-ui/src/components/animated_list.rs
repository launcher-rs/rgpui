...
/// 动画列表组件：用 `child_keyed` 注册项，状态驱动进出场动画。
#[derive(IntoElement)]
    ...
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let elements: Vec<AnyElement> = visible
            ...
            .filter_map(|(key, entering, exiting)| {
                children_by_key.remove(&key).map(|element| {
                    if entering {
                        div()
                            .child(element)
                            .with_animation(
        ...
        let mut list = div().id(self.id).flex().flex_col().children(elements);
        list.style().refine(&user_style);
        list
    }
}
