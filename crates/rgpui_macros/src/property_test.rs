...
pub fn test(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let Ok(func) = parse2::<ItemFn>(item) else {
        return quote_spanned! { item_span =>
            compile_error!("#[rgpui::property_test] must be placed on a function");
        };
    };
    ...
    let proptest_args = quote! {
        #[strategy = ::rgpui::seed_strategy()] __seed: u64,
        #proptest_args
    };
    ...
    let run_test_body = match &asyncness {
        ...
        Some(_) => quote! {
            let foreground_executor = rgpui::ForegroundExecutor::new(std::sync::Arc::new(dispatcher.clone()));
            #cx_vars
...
impl Args {
    ...
    /// `config = ...` token 并添加对 `rgpui::apply_seed_to_config` 的调用。
    fn render(&self) -> TokenStream {
        let user_provided_config = match &self.config {
            None => quote! { ::rgpui::proptest::prelude::ProptestConfig::default() },
            Some(config) => config.into_token_stream(),
        };

        let fixed_config = quote!(::rgpui::apply_seed_to_proptest_config(#user_provided_config));
        let remaining_args = &self.remaining_args;
        let errors = &self.errors;

        quote! {
            #errors
            #[::rgpui::proptest::property_test(
                proptest_path = "::rgpui::proptest",
                config = #fixed_config,
                #(#remaining_args,)*
            )]
        }
        ...
        for pair in pairs {
            match pair.path.get_ident().map(Ident::to_string).as_deref() {
                ...
                Some("proptest_path") => errors.extend(quote_spanned! {pair.span() =>
                    compile_error!("`rgpui::property_test` overrides the `proptest_path` parameter")
                }),
...
fn remove_background_executor(parsed: &mut ParsedArgs, args: &mut Vec<FnArg>) {
    args.retain_mut(|arg| {
        ...
        parsed
            .inner_fn_args
            .extend(quote!(rgpui::BackgroundExecutor::new(std::sync::Arc::new(
                dispatcher.clone()
            )),));
