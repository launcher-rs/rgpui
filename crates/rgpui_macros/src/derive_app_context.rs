...
/// 为持有 `&mut App` 的结构体生成 `rgpui::AppContext` 特质实现。
///
...
pub fn derive_app_context(input: TokenStream) -> TokenStream {
    ...
    let r#gen = quote! {
        impl #impl_generics rgpui::AppContext for #type_name #type_generics
        #where_clause
        {
            fn new<T: 'static>(
                &mut self,
                build_entity: impl FnOnce(&mut rgpui::Context<'_, T>) -> T,
            ) -> rgpui::Entity<T> {
                self.#app_variable.new(build_entity)
            }

            fn reserve_entity<T: 'static>(&mut self) -> rgpui::Reservation<T> {
                self.#app_variable.reserve_entity()
            }

            fn insert_entity<T: 'static>(
                &mut self,
                reservation: rgpui::Reservation<T>,
                build_entity: impl FnOnce(&mut rgpui::Context<'_, T>) -> T,
            ) -> rgpui::Entity<T> {
            ...
            fn update_entity<T, R>(
                &mut self,
                handle: &rgpui::Entity<T>,
                update: impl FnOnce(&mut T, &mut rgpui::Context<'_, T>) -> R,
            ) -> R
            where
                T: 'static,
            {
            ...
            fn as_mut<'y, 'z, T>(
                &'y mut self,
                handle: &'z rgpui::Entity<T>,
            ) -> rgpui::GpuiBorrow<'y, T>
            where
                T: 'static,
            {
            ...
            fn read_entity<T, R>(
                &self,
                handle: &rgpui::Entity<T>,
                read: impl FnOnce(&T, &rgpui::App) -> R,
            ) -> R
            where
                T: 'static,
            {
            ...
            fn update_window<T, F>(&mut self, window: rgpui::AnyWindowHandle, f: F) -> rgpui::Result<T>
            where
                F: FnOnce(rgpui::AnyView, &mut rgpui::Window, &mut rgpui::App) -> T,
            {
            ...
            fn with_window<R>(
                &mut self,
                entity_id: rgpui::EntityId,
                f: impl FnOnce(&mut rgpui::Window, &mut rgpui::App) -> R,
            ) -> Option<R>
            {
            ...
            fn read_window<T, R>(
                &self,
                window: &rgpui::WindowHandle<T>,
                read: impl FnOnce(rgpui::Entity<T>, &rgpui::App) -> R,
            ) -> rgpui::Result<R>
            where
                T: 'static,
            {
            ...
            fn background_spawn<R>(&self, future: impl std::future::Future<Output = R> + Send + 'static) -> rgpui::Task<R>
            where
                R: Send + 'static,
            {
            ...
            fn read_global<G, R>(&self, callback: impl FnOnce(&G, &rgpui::App) -> R) -> R
            where
                G: rgpui::Global,
            {
