...
GPUI 使用的过程宏（procedural macros）集合。

## 简介

`gpui_macros` 是 [GPUI](https://github.com/zed-industries/zed) 框架的配套宏库，提供了一系列过程宏，用于简化 GPUI 应用开发中的常见模式，包括组件渲染、上下文管理、动作定义、样式生成以及测试支持。
...
| 宏 | 说明 |
|---|---|
| `register_action!` | 手动实现 `Action` trait 时用于向 GPUI 运行时注册动作 |
...
| 宏 | 说明 |
|---|---|
| `#[gpui::test]` | 用于标注支持 GPUI 的测试函数，支持同步/异步测试、种子控制、迭代测试等 |
| `#[gpui::property_test]` | 支持基于属性的测试（property-based testing），集成 proptest 并支持值收缩（shrinking） |
...
`gpui_macros` 是 `gpui` crate 的底层依赖，为其提供核心的宏支持。两者关系如下：

- **gpui**：GPUI 框架的主体，提供 UI 组件、布局引擎、事件系统等核心功能
- **gpui_macros**：为 gpui 提供过程宏支持，包括：
  - 组件渲染和元素转换的派生宏
  - 上下文管理的派生宏
  - 动作系统的派生宏
  - 样式方法生成的函数宏
...
在项目中使用时，通常只需要依赖 `gpui` crate，宏会通过 `gpui` 重新导出。例如：
