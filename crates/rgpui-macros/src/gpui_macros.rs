...
/// `#[rgpui::test]` can be used to annotate test functions that run with GPUI support.
///
...
/// # Arguments
///
/// - `#[rgpui::test]` with no arguments runs once with the seed `0` or `SEED` env var if set.
/// - `#[rgpui::test(seed = 10)]` runs once with the seed `10`.
/// - `#[rgpui::test(seeds(10, 20, 30))]` runs three times with seeds `10`, `20`, and `30`.
/// - `#[rgpui::test(iterations = 5)]` runs five times, providing as seed the values in the range `0..5`.
/// - `#[rgpui::test(retries = 3)]` runs up to four times if it fails to try and make it pass.
/// - `#[rgpui::test(on_failure = "crate::test::report_failure")]` will call the specified function after the
///   tests fail so that you can write out more detail about the failure.
///
/// You can combine `iterations = ...` with `seeds(...)`:
/// - `#[rgpui::test(iterations = 5, seed = 10)]` is equivalent to `#[rgpui::test(seeds(0, 1, 2, 3, 4, 10))]`.
/// - `#[rgpui::test(iterations = 5, seeds(10, 20, 30)]` is equivalent to `#[rgpui::test(seeds(0, 1, 2, 3, 4, 10, 20, 30))]`.
/// - `#[rgpui::test(seeds(10, 20, 30), iterations = 5]` is equivalent to `#[rgpui::test(seeds(0, 1, 2, 3, 4, 10, 20, 30))]`.
///
...
/// `#[rgpui::bench]` annotates a Criterion benchmark that runs with GPUI support.
///
/// Use `#[rgpui::bench(inputs = some_iterable())]` on benchmarks that take an
...
///
/// The benchmark crate must add `criterion` and `rgpui_platform` (with its
/// `test-support` feature) to its dev-dependencies and enable rgpui's `bench`
...
/// A variant of `#[rgpui::test]` that supports property-based testing.
///
...
/// some of the same GPUI-specific arguments as `#[rgpui::test]`. Specifically,
...
/// `Arbitrary`. Parameters to a `#[rgpui::property_test]`, by default, use a
...
/// ## Scheduler
///
/// Similar to `#[rgpui::test]`, this macro will choose random seeds for the test
/// crate::scheduler. It uses `.no_shrink()` to tell proptest that all seeds are
/// roughly equivalent in terms of "complexity". If `$SEED` is set, it will
/// affect **ONLY** the seed passed to the crate::scheduler. To control other values,
