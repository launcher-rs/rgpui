...
pub fn bench(args: TokenStream, function: TokenStream) -> TokenStream {
    ...
    if !args.is_empty() {
        let parser = syn::meta::parser(|meta| {
            if meta.path.is_ident("fps") {
                ...
                if value == 0 {
                    return Err(meta.error("#[rgpui::bench] `fps` must be greater than zero"));
                }
                fps = Some(value);
                Ok(())
                ...
                if value == 0 {
                    return Err(
                        meta.error("#[rgpui::bench] `sample_size` must be greater than zero")
                    );
                }
                ...
                Err(meta.error(
                    "#[rgpui::bench] only accepts `fps = N`, `inputs = EXPR`, `input_name = \"...\"`, `group = \"...\"`, and `sample_size = N`",
                ))
            }
        });
    ...
    let report_expr = match fps {
        Some(fps) => quote! { rgpui::BenchReport::with_fps(#fps) },
        None => quote! { rgpui::BenchReport::default() },
    };
    ...
    let benchmark = if let Some(inputs) = inputs {
        ...
        quote! {
            ...
            for input in #inputs {
                group.bench_with_input(criterion::BenchmarkId::new(#input_name, &input), &input, {
                    ...
                    move |bencher, input| {
                        let mut cx = rgpui::BenchAppContext::new_with_platform_and_report(
                            rgpui::bench_platform(
                                Some(Box::new(|| {
                                    rgpui_platform::current_headless_renderer()
                                })),
                                rgpui_platform::current_platform(true).text_system(),
                            ),
        ...
        if let Some(input_name) = input_name {
            return error_to_stream(syn::Error::new(
                input_name.span(),
                "#[rgpui::bench] `input_name` requires `inputs`",
            ));
        }
        if let Some(group_name) = group_name {
            return error_to_stream(syn::Error::new(
                group_name.span(),
                "#[rgpui::bench] `group` requires `inputs`",
            ));
        }
        if sample_size.is_some() {
            return error_to_stream(syn::Error::new(
                proc_macro2::Span::call_site(),
                "#[rgpui::bench] `sample_size` requires `inputs`",
            ));
        }
        quote! {
            let report = #report_expr;
