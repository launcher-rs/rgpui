...
///
/// 生成的枚举提供：
/// - `path()`：返回图标在资源中的路径字符串（`icons/xxx.svg`）
/// - `bytes()`：返回通过 `include_bytes!` 在编译期嵌入的 SVG 原始字节
///
/// # 示例
///
/// ```ignore
...
/// ```
pub fn icon_named_impl(input: TokenStream) -> TokenStream {
    let IconNameInput {
        enum_name,
        path,
        derives,
    ...
    // 生成 include_bytes! 的字节嵌入表达式。
    // 使用 `env!("CARGO_MANIFEST_DIR")` 构造绝对路径，避免相对路径受调用文件位置影响。
    let byte_exprs: Vec<proc_macro2::TokenStream> = entries
        .iter()
        .map(|(_, p)| {
            let filename = p.trim_start_matches("icons/");
            let include = format!("{}/{}", relative_path, filename);
            quote! { include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/", #include)) }
        })
        .collect();
