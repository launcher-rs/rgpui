...
pub trait RopeExt {
    ...
    /// assert_eq!(rope.slice_line(2).to_string(), "This is a test 涓枃");
    ...
    /// assert_eq!(rope.slice_lines(2..5).to_string(), "This is a test 涓枃\nRope");
    ...
    /// assert_eq!(lines, vec!["Hello", "World\r", "This is a test 涓枃", "Rope"]);
    /// ```
    ...
    /// assert_eq!(rope.line_len(2), 21); // "This is a test 涓枃"
    ...
    /// use sum_tree::Bias;
    ///
    /// let rope = Rope::from("Hello 涓枃馃帀 test\nRope");
    /// assert_eq!(rope.clip_offset(5, Bias::Left), 5);
    /// // Inside multi-byte character '涓? (3 bytes)
    ...
    /// ```
    /// use rgpui_component::{Rope, RopeExt};
    /// let rope = Rope::from("a 涓枃馃帀 test\nRope");
    /// assert_eq!(rope.char_index_to_offset(0), 0);
    /// assert_eq!(rope.char_index_to_offset(1), 1);
    /// assert_eq!(rope.char_index_to_offset(3), "a 涓?.len());
    /// assert_eq!(rope.char_index_to_offset(5), "a 涓枃馃帀".len());
    /// ```
...
mod tests {
    ...
    fn test_lines() {
        ...
        assert_eq!(
            lines,
            vec!["Hello", "World\r", "This is a test 涓枃", "Rope\r"]
        );
    }
    ...
    fn test_iter_lines() {
        ...
        let lines: Vec<_> = rope
            ...
            .map(|r| r.to_string())
            .collect();
        assert_eq!(lines, vec!["World\r", "This is a test 涓枃"]);
    }
    ...
    fn test_line_column() {
        let rope = Rope::from("a 涓枃馃帀 test\nRope");
        assert_eq!(rope.position_to_offset(&Position::new(0, 3)), "a 涓?.len());
        assert_eq!(
        ...
        assert_eq!(
            rope.offset_to_position("a 涓枃馃帀 test\nR".len()),
            Position::new(1, 1)
        );
        assert_eq!(
            rope.offset_to_position("a 涓枃馃帀".len()),
            Position::new(0, 5)
        );
    }
    ...
    fn test_offset_to_point() {
        ...
        assert_eq!(rope.offset_to_point("a 涓?.len()), Point::new(0, 5));
        assert_eq!(rope.offset_to_point("a 涓枃馃帀".len()), Point::new(0, 12));
        assert_eq!(
            rope.offset_to_point("a 涓枃馃帀 test\nR".len()),
            Point::new(1, 1)
        );
    }
    ...
    fn test_point_to_offset() {
        ...
        assert_eq!(rope.point_to_offset(Point::new(0, 5)), "a 涓?.len());
        assert_eq!(rope.point_to_offset(Point::new(0, 12)), "a 涓枃馃帀".len());
        assert_eq!(
    ...
    fn test_word_at() {
        ...
        assert_eq!(rope.word_at(31), "涓枃");
        assert_eq!(rope.word_range(31), Some(28..34));
        assert_eq!(rope.word_at(38), "涓栫晫");
    ...
    fn test_offset_utf16_conversion() {
        ...
        assert_eq!(rope.offset_to_offset_utf16("hello 涓?.len()), 7);
        assert_eq!(rope.offset_to_offset_utf16("hello 涓枃".len()), 8);
        assert_eq!(rope.offset_to_offset_utf16("hello 涓枃馃帀".len()), 10);
        assert_eq!(rope.offset_to_offset_utf16(100), 20);

        assert_eq!(rope.offset_utf16_to_offset(5), "hello".len());
        assert_eq!(rope.offset_utf16_to_offset(7), "hello 涓?.len());
        assert_eq!(rope.offset_utf16_to_offset(8), "hello 涓枃".len());
        assert_eq!(rope.offset_utf16_to_offset(10), "hello 涓枃馃帀".len());
    ...
    fn test_replace() {
        ...
        let mut rope = Rope::from("涓枃");
        rope.replace(0..1, "New");
        // autocorrect-disable
        assert_eq!(rope.to_string(), "New鏂?);
        let mut rope = Rope::from("涓枃");
    ...
    fn test_clip_offset() {
        ...
        // Inside multi-byte character '馃帀' (4 bytes)
    ...
    fn test_char_index_to_offset() {
        ...
        assert_eq!(rope.char_index_to_offset(3), "a 涓?.len());
        assert_eq!(rope.char_index_to_offset(5), "a 涓枃馃帀".len());
        assert_eq!(rope.char_index_to_offset(6), "a 涓枃馃帀 ".len());
