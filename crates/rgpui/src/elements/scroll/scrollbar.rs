...
/// 滚动条的宽度（THUMB_ACTIVE_INSET * 2 + THUMB_ACTIVE_WIDTH）
...
impl ScrollbarAxis {
    ...
    fn all(&self) -> Vec<Axis> {
        match self {
            ...
            // 先水平后垂直，垂直是主轴；如果垂直无需显示，水平就不会保留右侧边距。
...
pub struct Scrollbar {
    ...
    axis: ScrollbarAxis,
    scrollbar_show: Option<crate::theme::ScrollbarShow>,
    scroll_handle: Rc<dyn ScrollbarHandle>,
    scroll_size: Option<Size<Pixels>>,
    /// 拖动滚动时每秒的最大帧数。默认 120 FPS。
    ///
    /// 用于限制拖动滚动条时对复杂交互的更新频率，降低 CPU 占用。
    max_fps: usize,
}
...
impl Scrollbar {
    ...
    /// 设置特定元素 id，默认为 [`Location::caller`]。
    ///
    /// 注意：大多数情况下无需为滚动条设置特定 id。
    pub fn id(mut self, id: impl Into<ElementId>) -> Self {
        self.id = id.into();
        self
    }

    /// 设置滚动条显示模式 [`ScrollbarShow`]，未设置时使用 `cx.theme().scrollbar_show`。
    pub fn scrollbar_show(mut self, scrollbar_show: crate::theme::ScrollbarShow) -> Self {
        self.scrollbar_show = Some(scrollbar_show);
        self
    }

    /// 设置内容区域的特殊滚动尺寸，默认为 None。
    ///
    /// 默认会从 `scroll_handle` 同步 `content_size`。
    ...
    /// 设置拖动滚动时每秒的最大帧数。默认 120 FPS。
    ///
    /// 若 CPU 占用过高，可考虑调低该值以提升性能。
    ///
    /// 可用值：30..120
    ...
    fn style_for_normal(&self, cx: &App) -> (Background, Hsla, Hsla, Pixels, Pixels, Pixels) {
        ...
        let (width, inset, radius) = match scrollbar_show {
            crate::theme::ScrollbarShow::Scrolling => (THUMB_WIDTH, THUMB_INSET, THUMB_RADIUS),
                ...
                for state in prepaint.states.iter() {
                    ...
                    window.on_mouse_event({
                        ...
                        move |event: &MouseMoveEvent, _, _, cx| {
                            ...
                            // 悬停显示模式或滚动条可见时，需要更新悬停状态并刷新 last_scroll_time。
