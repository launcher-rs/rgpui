...
impl LineWrapper {
    ...
    /// 将文本截断以适合指定数量的换行行。
    ///
    /// 与 `truncate_line`（将文本视为扁平宽度预算 `width * max_lines`）不同，
    /// 此方法考虑单词边界换行：它一次遍历字符，同时跟踪换行边界和截断点。
    /// 当文本在最后允许的行上溢出时，在此处截断并追加后缀。
    ///
    /// 对于 `max_lines == 1`，委托给 `truncate_line`。
