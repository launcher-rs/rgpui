...
use rgpui::{AnyElement, IntoElement, ParentElement, Styled, Window, div, px};

use rgpui_yororen_ui::component::{
    SelectionMode, TreeNode, TreeState, button, context_menu_trigger, empty_state, label, tree,
};
use rgpui_yororen_ui::theme::{ActionVariantKind, Theme};
use rgpui_yororen_ui::widget::virtual_list_state;
...
impl FileBrowserTreePanel {
    ...
    pub fn render(
        ...
        let tree_view: AnyElement = if is_empty {
            ...
            let list_state =
                virtual_list_state(tree_nodes.len(), rgpui::ListAlignment::Top, px(32.));
